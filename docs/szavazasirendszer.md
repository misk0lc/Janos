...
## 2. API autentikáció (Bearer token - JWT)

### 2.1 Regisztráció és bejelentkezés (JWT)

**Controller:** `app/Http/Controllers/Api/AuthApiController.php`

Ez a projekt a JWT (JSON Web Token) alapú Bearer tokeneket használja. A tokeneket a `php-open-source-saver/jwt-auth` csomag kezeli.

Példa regisztráció / bejelentkezés válasz:
...
- A kliens a védett végpontoknál a token-t az `Authorization: Bearer {token}` fejlécben küldi.
- A `logout` végpont érvényteleníti az aktuális token-t.
...
```
## 8. Adminisztrátor funkciók

### 8.1 Admin fiók bejelentkezés
...
### 8.2 Admin műveletek szavazásokon
...
### 9.1 Szavazás soft delete-elése

**Endpoint:** `DELETE /api/admin/polls/{poll_id}`

**Funkció:** A szavazás soft delete-re kerül, vagyis nem kerül véglegesen eltávolításra az adatbázisból.
...
### 9.2 Törölt szavazások listázása
...
### 9.3 Soft delete-elt szavazás visszaállítása
...
## 10. Végleges törlés (force delete)

### 10.1 Szavazás végleges törlése
